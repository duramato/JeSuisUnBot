...
def get_cmd(cmd, cmds_list):
    ...
    try: # the command's module needs to be imported from 'plugins/'
        mod = 'plugins.' + cmd
        mod = __import__(mod, globals(), locals(), [cmd])
    except ImportError as e: # inexistent module
        logging.error(helpers.C_INEXISTENT.format(cmd) + str(e))
        return None
    ...
    try:
        ...
        logging.error(helpers.C_INVALID.format(cmd) + str(e))
        return None
...
def run_cmd(sock, executor, to, cmd, arguments):
    ...
    def cb(f):
        try:
            ...
            response = helpers.C_EXCEPTION.format(cmd.__name__)
            logging.error(e)
